	...
	export default {
		...
		computed: {
			changlianQRCodeUrl: function() {
				return GLOBAL.env === 'test' ? GLOBAL.resourcePath + '/static/img/changlianQRCodeTest.jpg' : GLOBAL.resourcePath + '/static/img/changlianQRCode.jpg';
			}
		},
		created() {
			let _this = this;
					...
					if (res.code === 200) {
						...
						//跳转到电站详情页面
							...
							_this.$router.replace({
								...
								params: {
									stationId: res.body.paramObj.stationId
								}
							});
						}
						//跳转注册电站信息页面
						else if (res.body.pageName === 'registerStationInfo') {
							// 将 维护电站的二维码 存入 session.
							localStorage.setItem('registerStationInfoQRCode', _this.$route.params.qrCodeId);
							_this.$router.replace({
						...
						//跳转到注册终端
							...
							// 将 维护终端的二维码 存入 session.
							localStorage.setItem('registerTerminalInfoQRCode', _this.$route.params.qrCodeId);
							//跳转到新建页面
							_this.$router.replace({
								name: 'registerTerminalInfo',
								params: {
									qrCodeId: res.body.qrCodeId
								}
							});
						}
						//跳转到选择充电模式页面
							...
							_this.$router.replace({
								...
								params: {
									stationId: res.body.paramObj.stationId || '',
									portId: res.body.paramObj.portId || '',
									portNumber: res.body.paramObj.portNumber || '',
									consoleId: res.body.paramObj.consoleId || '',
									consoleNumber: res.body.paramObj.consoleNumber || ''
								}
							});
						}
					}
